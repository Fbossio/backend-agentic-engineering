# Progreso del curso

**Curso:** Backend Agentic Engineering  
**Versión del seguimiento:** 0.1  
**Última actualización:** 17 de julio de 2026  

---

## 1. Estado general

**Fase actual:** Diseño y gobierno  
**Estado:** En preparación  
**Próximo objetivo:** Aprobar documentos maestros e iniciar el Módulo 1.

---

## 2. Tablero de módulos

| N.º | Módulo | Esquema | Contenido | Laboratorio | Evaluación | Estado |
|---:|---|:---:|:---:|:---:|:---:|---|
| 1 | Del copiloto al desarrollo dirigido por agentes | Sí | No | No | No | Pendiente |
| 2 | Comunicación y especificaciones para agentes | Sí | No | No | No | Pendiente |
| 3 | ChatGPT para el ciclo completo de desarrollo | Sí | No | No | No | Pendiente |
| 4 | Investigación técnica verificable | Sí | No | No | No | Pendiente |
| 5 | Codex en ChatGPT, editor y terminal | Sí | No | No | No | Pendiente |
| 6 | Repositorios preparados para agentes | Sí | No | No | No | Pendiente |
| 7 | Delegación de funcionalidades completas | Sí | No | No | No | Pendiente |
| 8 | Debugging, refactorización y mantenimiento | Sí | No | No | No | Pendiente |
| 9 | Trabajo paralelo con varios agentes | Sí | No | No | No | Pendiente |
| 10 | Code review y control de calidad | Sí | No | No | No | Pendiente |
| 11 | OpenAI API para backend | Sí | No | No | No | Pendiente |
| 12 | Herramientas y function calling | Sí | No | No | No | Pendiente |
| 13 | Agents SDK y orquestación programática | Sí | No | No | No | Pendiente |
| 14 | MCP e integraciones | Sí | No | No | No | Pendiente |
| 15 | Seguridad en desarrollo agentic | Sí | No | No | No | Pendiente |
| 16 | Evaluación, observabilidad y costos | Sí | No | No | No | Pendiente |
| 17 | Flujo diario de un backend developer con agentes | Sí | No | No | No | Pendiente |
| 18 | Adopción en equipos y organizaciones | Sí | No | No | No | Pendiente |

---

## 3. Documentos maestros

| Documento | Estado | Próxima revisión |
|---|---|---|
| `CURSO_MASTER.md` | Versión inicial creada | Antes de iniciar el Módulo 1 |
| `PROGRESO.md` | Activo | Al cerrar cada sesión relevante |
| `DECISIONES.md` | Versión inicial creada | Cuando se tome una decisión global |
| `PLANTILLA_MODULO.md` | Versión inicial creada | Después de probarla con el Módulo 1 |
| `PROYECTO_TRANSVERSAL.md` | Borrador inicial | Antes del primer laboratorio acumulativo |

---

## 4. Hitos

### Hito 0 — Gobierno del curso

- [x] Crear Proyecto de ChatGPT.
- [x] Nombrar el chat principal `00 - Diseño y gobierno del curso`.
- [x] Definir el esquema general.
- [x] Crear documentos maestros.
- [ ] Revisar y aprobar documentos maestros.
- [ ] Crear repositorio Git.
- [ ] Subir los documentos al repositorio.
- [ ] Crear el chat `01 - Del copiloto al desarrollo dirigido por agentes`.

### Hito 1 — Fundamentos

- [ ] Completar Módulo 1.
- [ ] Completar Módulo 2.
- [ ] Realizar diagnóstico inicial.
- [ ] Definir métricas personales de comparación.

### Hito 2 — Uso profesional de ChatGPT y Codex

- [ ] Completar Módulos 3 a 8.
- [ ] Preparar un repositorio agent-ready.
- [ ] Delegar una funcionalidad vertical completa.

### Hito 3 — Coordinación y construcción de agentes

- [ ] Completar Módulos 9 a 14.
- [ ] Construir un flujo multiagente.
- [ ] Integrar OpenAI API.
- [ ] Crear herramientas y un agente programático.

### Hito 4 — Ingeniería responsable

- [ ] Completar Módulos 15 y 16.
- [ ] Implementar política de seguridad.
- [ ] Crear evaluaciones automatizadas.

### Hito 5 — Adopción y proyecto final

- [ ] Completar Módulos 17 y 18.
- [ ] Completar proyecto final.
- [ ] Comparar evaluación inicial y final.
- [ ] Publicar manual personal agent-first.

---

## 5. Próximas acciones

1. Revisar los cinco documentos maestros.
2. Confirmar el stack definitivo del proyecto transversal.
3. Crear el repositorio `backend-agentic-engineering`.
4. Crear la estructura inicial de carpetas.
5. Iniciar el desarrollo del Módulo 1.
6. Ejecutar la evaluación diagnóstica antes de introducir el flujo con agentes.

---

## 6. Registro breve de sesiones

### 17 de julio de 2026 — Diseño inicial

**Completado:**

- Definición del propósito.
- Estructura de 18 módulos.
- Estrategia de múltiples chats.
- Definición de documentos maestros.
- Creación de versiones iniciales.

**Decisiones relevantes:**

- Este chat será el espacio de gobierno.
- Los módulos se desarrollarán en chats separados.
- Los resultados aprobados se consolidarán en Git.
- El proyecto usará un laboratorio transversal.

### Hito 0 — Gobierno del curso

- [x] Crear Proyecto de ChatGPT.
- [x] Nombrar el chat principal `00 - Diseño y gobierno del curso`.
- [x] Definir el esquema general.
- [x] Crear documentos maestros.
- [x] Revisar y aprobar documentos maestros.
- [x] Crear repositorio Git.
- [x] Subir los documentos al repositorio.
- [x] Conectar GitHub con ChatGPT.
- [x] Configurar GitHub como fuente de verdad en las instrucciones.
- [ ] Crear el chat `01 - Del copiloto al desarrollo dirigido por agentes`.